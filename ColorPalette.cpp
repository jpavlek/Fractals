...
namespace Colors
{
	...
	ColorPalette::ColorPalette(const std::string& paletteName, const int paletteSize, const std::vector<Color>& colorPalette) noexcept:
	...
	ColorPalette::ColorPalette(std::string&& paletteName, const int paletteSize, std::vector<Color>&& colorPalette) noexcept:
		paletteName_{ std::move(paletteName) },
		paletteSize_{ paletteSize },
		colorPalette_{ std::move(colorPalette) }
	{
	}
	...
	void ColorPalette::saveColorPaletteAsBitmapFile(std::string fileName) noexcept
	{
		...
		FileSystem::Bitmap bitmap(width, DEFAULT_PALETTE_BITMAP_HEIGHT);
